...
def crete_file_share_if_not_exists(share_name):
    created = False
...
def create_dataset_if_not_exists(user, datastore, workspace):
...
def create_datastore_and_set_for_userspace_if_not_exists(user):
    ...
    try:
        ...
        crete_file_share_if_not_exists(share_name)
    ...
    create_dataset_if_not_exists(user, homespace_datastore, ws)


def mount_user_ds_on_ci(ci):
    HERE = os.path.join(os.path.dirname(__file__))
    ssh_port = ci.ssh_port
    ssh_host = f"azureuser@{ci.public_ip_address}"
    ds_name = dataset_properties(user).name
    copy_scripts_to_ci_cmd = ["scp", "-P", str(ssh_port), f"{HERE}/mount.sh", f'{HERE}/mount_ds.py', f"{ssh_host}:/tmp"]
    chmod_scripts = ["ssh", "-p", str(ssh_port), str(ssh_host), "chmod +x /tmp/mount.sh"]
    run_mount_scripts_cmd = ["ssh", "-t", "-p", str(ssh_port), str(ssh_host), f'bash -l -c "USER_DATASET_NAME={ds_name} /tmp/mount.sh"']
